...
def graph(df):
    ...
    for i, (porte, quantidade) in enumerate(
        labels[["porte", "quantidade"]].itertuples(index=False)
    ):
        ...
        data.append(
            go.Pie(
                ...
                values=[soma_total, quantidade],
                name=porte.capitalize(),
            )
        )
    ...
    fig = make_subplots(
        rows=1,
        cols=len(labels),
        specs=[[{"type": "domain"}] * len(labels)],
        subplot_titles=labels["porte"],
    )
