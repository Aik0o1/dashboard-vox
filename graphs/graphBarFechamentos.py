...
def plotGraphFechamentos(df):
    ...
    dados_municipio_sorted = dados_municipio.sort_values(by='fechamentos', ascending=False)
    dados_municipio_melhores = dados_municipio_sorted.head()
    fig = px.bar(dados_municipio_melhores, x="fechamentos", y="name", orientation='h', title='Em fechamento',height=300)
