...
def load_and_prepare_data(df):
    ...
    total_aberturas = df["abertura"].count()
    total_fechamentos = df["fechamento"].count()
    ...
    merge_abertura_fechamento = pd.merge(
        aberturas_por_ano, fechamentos_por_ano, on="ano", how="outer"
    ).fillna(0)
